...
class ListOrderDesigning extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      search: '',
  ...
  renderListOrder = () => {
    const { pullRefeshing } = this.state;
    const listDesigningFilter = this.filterOrder();
    return (
      <FlatList
        data={listDesigningFilter}
  ...
  filterOrder = () => {
    const { listDesigning } = this.props;
    const { search } = this.state;
    if (search !== '') {
      return listDesigning.filter((item) => {
...
export default ListOrderDesigning;
